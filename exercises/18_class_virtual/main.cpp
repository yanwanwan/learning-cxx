...
int main(int argc, char **argv) {
    ...
    ASSERT(d.virtual_name() == 'C', MSG); // D 继承了 C 的 final 实现
    ASSERT(a.direct_name() == 'A', MSG);
    ASSERT(b.direct_name() == 'B', MSG);
    ASSERT(c.direct_name() == 'C', MSG);
    ASSERT(d.direct_name() == 'D', MSG);

    // 2. 引用访问：虚函数看物体，普通函数看标签

    A &rab = b; // Label: A, Object: B
    B &rbc = c; // Label: B, Object: C
    C &rcd = d; // Label: C, Object: D

    ASSERT(rab.virtual_name() == 'B', MSG);
    ASSERT(rbc.virtual_name() == 'C', MSG);
    ASSERT(rcd.virtual_name() == 'C', MSG); // D 使用 C 的实现
    
    ASSERT(rab.direct_name() == 'A', MSG); // 这里的 direct_name 只看标签类型
    ASSERT(rbc.direct_name() == 'B', MSG);
    ASSERT(rcd.direct_name() == 'C', MSG);

    A &rac = c; // Label: A, Object: C
    B &rbd = d; // Label: B, Object: D
